...
public class TagTextView extends android.support.v7.widget.AppCompatTextView {
    ...
    private int tagTextSize = 9;       //  标签的字体大小
    private int tagViewHeight = 16;       //  标签高度
     ...
     * 设置标签控件高度
     *
     * @param height tag view 的高度，单位为dp
     */
    public void setTagViewHeight(int height) {
        this.tagViewHeight = height;
    }
     ...
     * @param content 标签文字
     */
    public void setMultiTagAndContent(@NonNull List<String> tags, @NonNull List<Integer> tagBgs, String content) {
        if (tags.size() != tagBgs.size()) {
            throw new InvalidParameterException("tags size must equal tagBgs size");
        }
        if (tagsIndex == TAGS_INDEX_AT_START) {
            setTagStart(tags, tagBgs, content);
        } else {
            setTagEnd(tags, content);
        }
    }
     ...
     * @param content 标签文字
     */
    public void setTagStart(List<String> tags, List<Integer> tagBgs, String content) {
        int endIndex = 0;
        int startIndex = 1;
        ...
        for (int i = 0; i < tags.size(); i++) {
            ...
            layoutParams.height = dp2px(mContext, tagViewHeight);
            tv_tag.setLayoutParams(layoutParams);
            tv_tag.setTextColor(Color.parseColor(tagTextColor));
            //  设置背景样式
            tv_tag.setBackgroundResource(tagBgs.get(i));
    ...
    public void setTagEnd(List<String> tags, String content) {
        ...
        for (int i = 0; i < tags.size(); i++) {
            ...
            CenterImageSpan span = new CenterImageSpan(drawable);
            spannableString.setSpan(span, content_buffer.length() - item.length(), content_buffer.length(), Spannable
